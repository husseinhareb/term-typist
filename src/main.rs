...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    'mainloop: loop {
        terminal.draw(|f| {
            ...
            // Speed panel split: WPM | Timer/Counter
            ...
            // Right: Mode-specific display
            let right_content = match app.selected_tab {
                1 => {
                    // Words mode: count completed words
                    let current_idx = app
                        .status
                        .iter()
        ...
        if event::poll(timeout)? {
            if let Event::Key(KeyEvent { code, modifiers, .. }) = event::read()? {
                ...
                match code {
                    ...
                    KeyCode::Char(c) => {
                        if app.selected_tab != 2 { // Zen mode skips typing
                            app.on_key(c);
